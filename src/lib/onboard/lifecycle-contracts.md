...
## Onboarding live policy boundary

Onboarding may supply an initial policy or request policyless creation with `--apf-interceptor`. The create plan records only which delivery mode is required. After creation, OpenShell is the sole policy store: readiness verification reads the live policy and validates the stable requirements for the selected agent, provider, messaging channels, observability, GPU mode, and web search setup.

The completed session and registry contain no policy content, owner, hash, version, tier, preset list, custom-policy YAML, or baseline-exclusion journal. Older files containing those fields load with the fields ignored. A host-side policy source or version change is valid and does not block onboarding.

If post-create policy verification cannot read or validate the live result, onboarding preserves the sandbox and its identity-bound recovery state. Policy-dependent credential or provider attachment occurs only after the required live policy change is confirmed.
...
fresh OpenClaw onboard
  ...
  -> gateway setup -> checkpoint name/identity and web prompts*
  -> checkpoint messaging prompts*
  -> checkpoint resource prompt*
  -> resolve complete sandbox-create intent
  -> materialize initial policy -> create -> ready -> live policy validation
  -> create or update exact credential providers + save binding receipts*
  -> register
...
rebuild / installer upgrade
  registry (+ guarded non-policy session fallback) -> preflight -> capture live policy handoff*
  -> mutation-edge recheck
  -> ! delete -> recreate session* -> inner onboard create+register/session*
  -> outer restore -> exact live-policy restore -> MCP reconciliation -> delete handoff*
...
| **Rebuild or installer-driven upgrade** — `rebuildSandbox` in `rebuild-pipeline.ts`; `upgradeSandboxes` | Registry state is authoritative for managed resource and runtime configuration, never policy. Rebuild reads the current OpenShell policy and creates a mode-`0600`, hash-verified handoff for this operation. A matching session may fill guarded non-policy legacy gaps only when its selection agrees. | A missing or unreadable live policy stops the flow before **sandbox delete**. Target preflight, backup, Shields unlock, MCP detach/scrub, and NIM stop precede deletion. The replacement receives no registry preset replay; after state restore, NemoClaw writes and verifies the exact handed-off policy. | The backup manifest may retain only the bounded policy-handoff filename and digest while recovery is incomplete. It contains no reusable preset list or custom-policy copy. Credential metadata comes from the target or guarded fallback; raw credentials/providers remain process- or gateway-bound. | Successful restore deletes the handoff. A failed or unverified policy restore keeps it for retry and reports the live result. In-process rollback best-effort restores registry/MCP retry metadata. Covered by rebuild, managed-workload authority, image-preflight, DCode, messaging, and exact-policy restore tests. |
...
| **Channel add/remove/start/stop** — `addSandboxChannel`, `removeSandboxChannel`, `sandboxChannelsSetEnabled` in `policy-channel.ts` | Add compiles a manifest-derived channel delta. Start, stop, and remove transform the registry messaging plan and rehydrate executable details from current manifests. Policy state is read from OpenShell. | Each operation applies or verifies its scoped live-policy delta first. Credential/provider attachment and messaging-plan persistence follow only after OpenShell readback confirms the change. A rejected or ambiguous-unconfirmed policy write stops later effects. A queued rebuild has a separate delete boundary. | The compact registry messaging plan is authoritative for messaging resources, not policy. No session or registry preset list is synchronized. Raw tokens stay in process/gateway bindings. | Rollback restores only effects owned by the channel transaction and preserves unrelated host policy changes. Covered by `policy-channel*.test.ts`, `workflow-planner.test.ts`, and channel integration tests. |
...
| **Config, policy, resource, port-forward, and runtime setup contributions** — `configSet`; initial policy planning; `selectResourceProfileForSandbox`; manifest compiler/runtime appliers; dashboard and channel forward helpers | Config uses validated dotpaths and SSRF-safe URL rewriting. Create contributions assemble an initial policy and non-policy runtime inputs; later policy changes always derive from the live OpenShell document. | Config’s first effect is a compare-and-swap sandbox write. Policy mutations use the live read/write/readback boundary. Build-time contributions inherit the enclosing create/recreate boundary. | Durable owners are compact messaging/inference/resource metadata, current manifests, onboard session, sandbox config/hash, gateway provider state, and bounded Shields recovery state. Policy desired state is never a durable owner. Logical credential bindings are serializable; raw values are not. | CAS rejects stale config writes. OpenShell rejection stops dependent provider and credential effects. Forward recovery can re-establish declared forwards. |
...
| Agent intent | `hermesAuthMethod`, `toolDisclosure`, `hermesToolGateways` | Agent setup owns these non-policy choices. Policy selections are transient inputs applied to OpenShell and are not written to the completed session. |
