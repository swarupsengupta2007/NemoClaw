...
describe("prepareOnboardSession", () => {
  it("stops resume before side effects when saved policy requirements is invalid (#9833)", async () => {
    const loadSession = vi.fn((): Session | null => {
      throw new Error(
        "Refusing to load the onboarding session: the saved policy requirements is invalid.",
      );
    });
    ...
    ).rejects.toThrow(/saved policy requirements is invalid/u);
